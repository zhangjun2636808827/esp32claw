...
static int cmd_led_off(int argc, char **argv)
{
    ...
    char output[128];
    esp_err_t err = tool_led_off_execute("{}", output, sizeof(output));
    printf("%s\n", output);
...
static int cmd_led_set(int argc, char **argv)
{
    ...
    int red = led_set_args.red->ival[0];
    int green = led_set_args.green->ival[0];
    int blue = led_set_args.blue->ival[0];
    int brightness = led_set_args.brightness->count ? led_set_args.brightness->ival[0] : 255;
    ...
    esp_err_t err = tool_set_led_color_execute(input, output, sizeof(output));
    printf("%s\n", output);
    return (err == ESP_OK) ? 0 : 1;
}
...
esp_err_t serial_cli_init(void)
{
    ...
    /* led_set */
    led_set_args.red = arg_int1(NULL, NULL, "<r>", "Red 0-255");
    led_set_args.green = arg_int1(NULL, NULL, "<g>", "Green 0-255");
    led_set_args.blue = arg_int1(NULL, NULL, "<b>", "Blue 0-255");
    led_set_args.brightness = arg_int0(NULL, NULL, "<brightness>", "Optional brightness 0-255");
    led_set_args.end = arg_end(2);
    esp_console_cmd_t led_set_cmd = {
        .command = "led_set",
        .help = "Set WS2812B RGB LED (e.g. led_set 255 0 0 [brightness])",
        .func = &cmd_led_set,
        .argtable = &led_set_args,
    };
    esp_console_cmd_register(&led_set_cmd);

    /* led_off */
    esp_console_cmd_t led_off_cmd = {
        .command = "led_off",
        .help = "Turn off the WS2812B RGB LED",
        .func = &cmd_led_off,
    };
